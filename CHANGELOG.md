...
평면 지도의 산맥 기호 ([wwolf 018](devlog/20260925_wwolf_018_mountain_marks.md))

- 고도 격자에서 ☰ 메뉴의 **3D 지형**이 **입체 지형**이 되었다. 지구본에서는 전과 같이 확대하면 땅을 올리고,
  몰바이데·이퀄 어스·정거원통에서는 산맥을 작은 삼각형 기호로 보인다(봉우리 높이에 따라 세 크기).
- 현재와 시간 창에서는 Natural Earth의 이름난 주요 산맥 29개(안데스, 로키, 히말라야, 대분수령, 알프스,
  우랄 등)의 능선을 따라, 과거에서는 그 시기 PaleoDEM 격자의 높이에서 찾은 산맥(주변보다 900 m 이상
  솟은 1000 m 이상의 땅)에 놓인다. 과거의 기호는 이름 붙은 산맥이 아니라 높이 자료에서 찾은 산이다.
- 기호는 지도와 함께 움직인다: 지도가 다 불러와진 뒤에 바뀌고, 두 격자 사이에서는 대륙과 같은 이동으로
  옮겨지며 교차 페이드한다.
- 스위치를 누르면 확대한 지구본의 땅이 곧바로 오르내린다(전에는 다음 이동·확대 때까지 그대로였다).

확인: 고도 격자 → 투영 **이퀄 어스** → ☰ 메뉴 **입체 지형**을 켠 채로. 0 Ma에서 안데스·히말라야를 따라
삼각형이, 250 Ma에서 판게아의 산지에 작은 삼각형이 보인다. 슬라이더를 55 → 50 Ma로 옮기면 인도의 기호가
땅과 함께 움직인다. 빌드: `scripts/fetch_paleodem.py --manifest sources/mountain-ranges.json` 뒤
`scripts/build_mountain_ranges.py`.
