...
- 시간 범위의 **모형 강수** 색을 청록 한 색상에서 ColorBrewer BrBG(건조 갈색 ↔ 습윤 청록, 500 mm가
  옅은 가운데)로 바꿨다. 사막이 거의 흰색으로 묻히지 않고 갈색으로 보인다. 녹색약·적색약에서도
  건조와 습윤이 겹치지 않는지 검사했다(devlog wwolf 016). 자료·범례 눈금·같은 색 = 같은 mm/년은 그대로다.

확인: 고도 격자 → 시간 범위 "최근 2.5만 년" → ☰ 메뉴 **모형 강수**. 0 ka에서 사하라·아라비아·아타카마가
갈색, 아마존·콩고가 청록이다. 범례 막대가 같은 색 순서다.

하천 표시를 "잠재 하천"으로

- ☰ 메뉴의 **하천** 버튼과 정보 패널 출처의 이름을 **잠재 하천**(영어 "Potential rivers")으로 바꿨다.
  파란 선은 강수와 무관하게 지형만으로 흘려 계산한 잠재 배수망이어서, 사하라 한가운데의 선이 그곳에 물이
  흐른다는 뜻으로 읽히지 않게 한다.
- **모형 강수** 범례 아래에 한 줄 주의를 붙였다: 선은 지형만으로 그린 물길이며, 사막을 가로지르는 선이
  물이 흐른다는 뜻은 아니고, 나일강처럼 먼 상류의 비로 사막을 건너는 강도 있다. 강수가 적은 곳의 선을
  지우지 않은 까닭이 이것이다(나일강까지 지워진다). 강수로 무게를 준 하천 계산은 별도의 빌드 작업이다.

확인: 고도 격자 → 시간 범위 "최근 2.5만 년" → ☰ 메뉴에서 **잠재 하천**, 그리고 **모형 강수**. 범례 아래에
주의 문장이 보인다. 영어 화면(`/en/`)에서는 "Potential rivers".

기온 색: 지구본과 범례가 같은 색으로, 색 아래에 산맥 음영 ([wwolf 017](devlog/20260924_wwolf_017_temperature_colours.md))

- ☰ 메뉴 **기온**의 색을 ColorBrewer RdYlBu(영하 파랑, 0 °C 옅은 색, 위로 노랑·주황·빨강)로 바꿨다.
  예전 색은 지구본과 범례 막대가 서로 다르게 섞여 같은 온도가 최대 ΔE2000 13만큼 다른 색이었다. 이제
  둘이 같은 색이고, 5 °C 차이가 적록 색각 이상에서도 두 배 이상 잘 갈린다. 범위(−30~40 °C)와 눈금은 같다.
- **기온**, **모형 식생**, **모형 강수**에서도 육지에 지형 음영이 들어가 산맥이 보인다. 평지는 범례의
  색 그대로이고 경사만 밝거나 어둡다. 정보 패널 **지형 음영**으로 세기를 바꾸거나 끈다.

확인: 고도 격자 → ☰ 메뉴 **기온** → 투영 **이퀄 어스**. 0 Ma에서 남극·그린란드가 파랑, 열대가 빨강이고,
히말라야·안데스에 음영이 있다. **지형 음영** "음영 없음"이면 평평해진다.

## v0.22.0 — 2026-09-24

투영에 이퀄 어스 추가 ([#86](https://github.com/jikhanjung/EarthThruTime3D/pull/86))

- 툴바 **투영** 선택에 **이퀄 어스**(Šavrič, Patterson & Jenny 2018)가 생겼다. 몰바이데와 같은
  정적도 도법이라 면적은 그대로지만, 극이 점이 아니라 적도 폭의 59 %인 선이라 고위도 대륙이 덜
  눌린다(60°에서 지도 폭 75 %, 몰바이데는 65 %; 75°에서 65 % 대 42 %). 지도 비율은 2.055:1이다.
- 격자·지명·판 경계·해안선·위치 핀은 다른 평면 지도와 같은 경로로 놓이고, 경선 돌리기·이동·확대도
  같다. 자료와 셰이더의 표본 추출은 바뀌지 않는다.

확인: 툴바 투영에서 **이퀄 어스**. 몰바이데와 번갈아 고르면 그린란드·시베리아·남극의 폭이 달라지는
것이 보인다. 시간 범위 "최근 13만 년"의 21 ka에서 빙상을 보면 차이가 가장 뚜렷하다.
