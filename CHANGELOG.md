...
- 투영, 표면(고도·기온·모형 식생·모형 강수 등), 지형 음영, 해수면, 3D 지형·하천·빙하·격자 스위치가 주소에
  실린다. 보기를 바꾸면 주소가 따라 바뀌고(기본값과 다른 것만), 그 주소를 열거나 보내면 같은 보기가 열린다.
  시점(`age`)도 슬라이더를 따라 적힌다. 표시 자료·시간 범위·위치 핀은 전과 같은 매개변수다.

확인: 고도 격자에서 투영 **이퀄 어스**, ☰ 메뉴 **기온**, 슬라이더를 300 Ma로. 주소에 `view=equalearth`,
`surface=temp`, `age=300`이 붙는다. 그 주소를 새 탭에서 열면 같은 보기다. 매개변수 표는 docs/site-map.md.
