...
범례는 지도 위에, 정보 패널은 접히게

- **기온**·**모형 식생**·**모형 강수**의 색 범례가 정보 패널 아래쪽 대신 지도 왼쪽 위에 떠 있다. 패널을
  열지 않아도 보이고, 색 보기를 끄면 사라진다.
- 정보 패널의 **지구 내부** 절이 제목 한 줄(▸)로 접힌다. 지각이나 맨틀을 켜 두지 않았으면 접힌 채
  시작하고, 펼치거나 접은 선택은 이 브라우저에 남는다. 그래서 시대·나이와 **지형 음영**이 패널 위쪽에 온다.
- 패널의 설명문(빙하, 하천, 해수면, 모형 기후, 판 등)이 두 줄로 접히고(▸), 누르거나 Enter로 펼친다.
  두 줄 안에 들어가는 짧은 글은 표시 없이 그대로다.

확인: 고도 격자 → ☰ 메뉴 **기온** 또는 시간 범위 "최근 2.5만 년"에서 **모형 강수**. 범례가 지도 왼쪽 위에
있다. **정보**를 열면 **▸ 지구 내부**가 한 줄이고, 설명문을 누르면 펼쳐진다.
