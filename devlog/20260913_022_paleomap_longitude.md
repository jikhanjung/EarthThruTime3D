...
측정 뒤 PALEOMAP을 뷰어에도 올렸다. Zenodo 기록의 이용 조건이 CC BY 4.0이라 공개 모델로 둔다.
`scripts/pack_paleomap.py`는 지우고 `sources/plate-models/paleomap2016.json` 목록 파일로 옮겼다.
회전 파일과 판 다각형만 멤버로 고정하고, 같은 묶음의 지도 이미지는 풀지 않는다. 기간이 0인
다각형 제거는 이 모델의 목록 파일에 `pack.drop_zero_span`으로 켰다. 다른 모델에도 그런 다각형이
몇 개씩 있지만 이미 배포된 묶음을 바꾸지 않으려고 전역으로 켜지 않았다. 필터 뒤 대륙 다각형이
274개라 이미지 검사의 하한을 300에서 250으로 낮췄다.
