...
이미지: **`honestjung/earththrutime3d:v0.18.1`**, 플랫폼 `linux/amd64`.
`../hanyang3d/deploy`의 Gunicorn·버전 이미지·Compose·상태 확인 구성을 참고했고,
데이터베이스가 있는 서비스이므로 백업과 복구 단계를 더했다.

2026-09-18 v0.18.1 운영 배포 완료. 이미지 ID:
`sha256:3c653a2c354c58a648ce647f86b0a1b1ff2d143a1f5af76163663288913cfc45`.
배포·백업·공개 화면 검증 결과는 [릴리스 기록](../devlog/20260918_jikhanjung_098_release_0181.md)에 있다.
...
- 데이터 묶음(`deploy/pack_data.py`):
  ...
  - PaleoMIST 1.0 빙하 하천 10장(2.5–25 ka, 2.5 ka 간격)과 시점 목록. 검증된 원본으로 호수 깊이 채널과 얼음 밖의 현재 하천·호수를 포함해 재생성.
  ...
  경로·크기·SHA-256을 `manifest.json`에 기록한다. v0.18.1 릴리스에는 고도 시리즈와
...
운영 자료의 빙기 하천 범위는 2.5–25 ka의 10장이다. `--ice --ice-to 25`로 생성하고
sidecar와 PNG 목록을 확인한다. 패커는 디렉터리에 있는 모든 빙기 PNG를 포함하므로,
`--ice-to 80`으로 만든 연구용 32장을 그대로 패킹하지 않는다(#59 별도).

## 배포 동작

| 동작 | 명령 |
|---|---|
| preflight·build | `bash deploy/build.sh v0.18.1` (개발 호스트) |
| deploy | `bash /srv/earththrutime3d/deploy.sh v0.18.1` |
...
```text
dist/earththrutime3d-image-v0.18.1.tar.gz
dist/earththrutime3d-data-v0.18.1.tar.gz
dist/earththrutime3d-host-v0.18.1.tar.gz
dist/SHA256SUMS-v0.18.1
```
...
scp dist/earththrutime3d-*-v0.18.1.tar.gz dist/SHA256SUMS-v0.18.1 dolfinid:~/earththrutime3d-release/
```
...
sha256sum -c SHA256SUMS-v0.18.1
docker load -i earththrutime3d-image-v0.18.1.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/earththrutime3d
tar -xzf earththrutime3d-host-v0.18.1.tar.gz -C /srv/earththrutime3d
mkdir -p /srv/earththrutime3d/data/v0.18.1 /srv/earththrutime3d/db \
         /srv/earththrutime3d/backups /srv/earththrutime3d/acme
tar -xzf earththrutime3d-data-v0.18.1.tar.gz -C /srv/earththrutime3d/data/v0.18.1
