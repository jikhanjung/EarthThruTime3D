...
이미지: **`honestjung/earththrutime3d:v0.9.3`**, 플랫폼 `linux/amd64`.
...
- 이미지 ID: `sha256:b2a1fce6eb5e20ab348c3e342ce4ca095cd577451fb97aff9582b6273d26c4e8` (v0.9.2, 2026-09-14 배포)
...
| 동작 | 명령 |
|---|---|
| preflight·build | `bash deploy/build.sh v0.9.3` (개발 호스트) |
| deploy | `bash /srv/earththrutime3d/deploy.sh v0.9.3` |
...
```text
dist/earththrutime3d-image-v0.9.3.tar.gz
dist/earththrutime3d-data-v0.9.3.tar.gz
dist/earththrutime3d-host-v0.9.3.tar.gz
dist/SHA256SUMS-v0.9.3
```
...
scp dist/earththrutime3d-*-v0.9.3.tar.gz dist/SHA256SUMS-v0.9.3 dolfinid:~/earththrutime3d-release/
```
...
sha256sum -c SHA256SUMS-v0.9.3
docker load -i earththrutime3d-image-v0.9.3.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/earththrutime3d
tar -xzf earththrutime3d-host-v0.9.3.tar.gz -C /srv/earththrutime3d
mkdir -p /srv/earththrutime3d/data/v0.9.3 /srv/earththrutime3d/db \
         /srv/earththrutime3d/backups /srv/earththrutime3d/acme
tar -xzf earththrutime3d-data-v0.9.3.tar.gz -C /srv/earththrutime3d/data/v0.9.3
