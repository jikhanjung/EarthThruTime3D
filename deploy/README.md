...
이미지: **`honestjung/earththrutime3d:v0.22.0`**, 플랫폼 `linux/amd64`.
`../hanyang3d/deploy`의 Gunicorn·버전 이미지·Compose·상태 확인 구성을 참고했고,
데이터베이스가 있는 서비스이므로 백업과 복구 단계를 더했다.

2026-09-24 v0.22.0 운영 배포 완료. 이미지 ID:
`sha256:55c1fc16cd0b5ab7d9706166847fc73c1d2fb1160a7c8cb9dc1942b2dcafc1a0`.
배포·백업·공개 화면 검증 결과는 [릴리스 기록](../devlog/20260924_jikhanjung_106_release_0220.md)에 있다.
...
- 데이터 묶음(`deploy/pack_data.py`):
  ...
  - 잠재 하천 필드 109장과 저수위 필드 31장(2048×1024 RGB), 현재 격자는 Natural Earth 하천 라우팅과 HydroLAKES 호수를 반영하고, 나머지는 green/blue=0.
  - PaleoMIST 1.0 빙하 하천 32장(2.5–80 ka, 2.5 ka 간격)과 시점 목록. 검증된 원본으로 호수 깊이 채널과 얼음 밖의 현재 하천·호수를 포함해 재생성.
  - 13만 년 창의 빙상 조각 80장: 1–25 ka는 NADI-1·DATED-1, 26–80 ka는 PaleoMIST 1.0 접지 빙상(1 ka 간격, 조각 발자국 3° 이내).
  - 시간 창의 모형 기후 131장(0–130 ka, 720×360, Krapp et al. 2021 CC BY 4.0): 식피 등급과 연 강수량. 빌드돼 있을 때만 포함한다.
  - 평면 지도의 산맥 표시 `mountain-ranges.json` 1개(약 700 KB): 현재는 Natural Earth 명명 산맥 29개, 과거 격자는 각 격자의 높이에서 찾은 산맥. 빌드돼 있을 때만 포함한다.
  ...
  경로·크기·SHA-256을 `manifest.json`에 기록한다. v0.22.0 릴리스에는 고도 시리즈와
  빙하 마스크 57장, 빙상 조각 80장, 모형 기후 131장, 12비트 0 Ma 고도 텍스처가 포함됐다. 정확한 파일 목록은 릴리스 묶음의 매니페스트를 따른다.
...
| 동작 | 명령 |
|---|---|
| preflight·build | `bash deploy/build.sh v0.22.0` (개발 호스트) |
| deploy | `bash /srv/earththrutime3d/deploy.sh v0.22.0` |
...
```text
dist/earththrutime3d-image-v0.22.0.tar.gz
dist/earththrutime3d-data-v0.22.0.tar.gz
dist/earththrutime3d-host-v0.22.0.tar.gz
dist/SHA256SUMS-v0.22.0
```
...
scp dist/earththrutime3d-*-v0.22.0.tar.gz dist/SHA256SUMS-v0.22.0 dolfinid:~/earththrutime3d-release/
```
...
sha256sum -c SHA256SUMS-v0.22.0
docker load -i earththrutime3d-image-v0.22.0.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/earththrutime3d
tar -xzf earththrutime3d-host-v0.22.0.tar.gz -C /srv/earththrutime3d
mkdir -p /srv/earththrutime3d/data/v0.22.0 /srv/earththrutime3d/db \
         /srv/earththrutime3d/backups /srv/earththrutime3d/acme
tar -xzf earththrutime3d-data-v0.22.0.tar.gz -C /srv/earththrutime3d/data/v0.22.0
