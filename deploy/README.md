...
이미지: **`honestjung/earththrutime3d:v0.10.3`**, 플랫폼 `linux/amd64`.
...
- 이미지 ID: `sha256:e94e5bf9f68c89a24dcb7610827868932c0d49c11ac5f7d17b8b9378ad00e789` (v0.10.3, 2026-09-15 배포)
...
- 데이터 묶음(`deploy/pack_data.py`):
  ...
  경로·크기·SHA-256을 `manifest.json`에 기록한다. v0.10.3 릴리스에는 고도 시리즈와
  빙하 마스크 57장, 저수위 조각 18장이 포함됐다. 정확한 파일 목록은 릴리스 묶음의 매니페스트를 따른다.
...
| 동작 | 명령 |
|---|---|
| preflight·build | `bash deploy/build.sh v0.10.3` (개발 호스트) |
| deploy | `bash /srv/earththrutime3d/deploy.sh v0.10.3` |
...
```text
dist/earththrutime3d-image-v0.10.3.tar.gz
dist/earththrutime3d-data-v0.10.3.tar.gz
dist/earththrutime3d-host-v0.10.3.tar.gz
dist/SHA256SUMS-v0.10.3
```
...
scp dist/earththrutime3d-*-v0.10.3.tar.gz dist/SHA256SUMS-v0.10.3 dolfinid:~/earththrutime3d-release/
```
...
sha256sum -c SHA256SUMS-v0.10.3
docker load -i earththrutime3d-image-v0.10.3.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/earththrutime3d
tar -xzf earththrutime3d-host-v0.10.3.tar.gz -C /srv/earththrutime3d
mkdir -p /srv/earththrutime3d/data/v0.10.3 /srv/earththrutime3d/db \
         /srv/earththrutime3d/backups /srv/earththrutime3d/acme
tar -xzf earththrutime3d-data-v0.10.3.tar.gz -C /srv/earththrutime3d/data/v0.10.3
