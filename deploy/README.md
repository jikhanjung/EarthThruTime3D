...
이미지: **`honestjung/earththrutime3d:v0.19.0`**, 플랫폼 `linux/amd64`.
`../hanyang3d/deploy`의 Gunicorn·버전 이미지·Compose·상태 확인 구성을 참고했고,
데이터베이스가 있는 서비스이므로 백업과 복구 단계를 더했다.

2026-09-18 v0.19.0 운영 배포 완료. 이미지 ID:
`sha256:206cf263c18c769dc1fb739601e49a7c53471dde96772c6678befb44ba26a18c`.
배포·백업·공개 화면 검증 결과는 [릴리스 기록](../devlog/20260918_jikhanjung_102_release_0190.md)에 있다.
...
- 데이터 묶음(`deploy/pack_data.py`):
  ...
  경로·크기·SHA-256을 `manifest.json`에 기록한다. v0.19.0 릴리스에는 고도 시리즈와
  빙하 마스크 57장, 빙상 조각 80장, 12비트 0 Ma 고도 텍스처가 포함됐다. 정확한 파일 목록은 릴리스 묶음의 매니페스트를 따른다.
...
운영 자료의 빙기 하천 범위는 v0.19.0부터 2.5–80 ka의 32장이다. `scripts/build_ice.py`
...
| 동작 | 명령 |
|---|---|
| preflight·build | `bash deploy/build.sh v0.19.0` (개발 호스트) |
| deploy | `bash /srv/earththrutime3d/deploy.sh v0.19.0` |
...
```text
dist/earththrutime3d-image-v0.19.0.tar.gz
dist/earththrutime3d-data-v0.19.0.tar.gz
dist/earththrutime3d-host-v0.19.0.tar.gz
dist/SHA256SUMS-v0.19.0
```
...
scp dist/earththrutime3d-*-v0.19.0.tar.gz dist/SHA256SUMS-v0.19.0 dolfinid:~/earththrutime3d-release/
```
...
sha256sum -c SHA256SUMS-v0.19.0
docker load -i earththrutime3d-image-v0.19.0.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/earththrutime3d
tar -xzf earththrutime3d-host-v0.19.0.tar.gz -C /srv/earththrutime3d
mkdir -p /srv/earththrutime3d/data/v0.19.0 /srv/earththrutime3d/db \
         /srv/earththrutime3d/backups /srv/earththrutime3d/acme
tar -xzf earththrutime3d-data-v0.19.0.tar.gz -C /srv/earththrutime3d/data/v0.19.0
