...
이미지: **`honestjung/earththrutime3d:v0.8.2`**, 플랫폼 `linux/amd64`.
...
- 이미지 ID: `sha256:3c139d1029f3017ade6ec7be8e0b6ecf60198edeef1b49c03001dd8f83bf1f98` (v0.8.2, 2026-09-13 배포)
...
| 동작 | 명령 |
|---|---|
| preflight·build | `bash deploy/build.sh v0.8.2` (개발 호스트) |
| deploy | `bash /srv/earththrutime3d/deploy.sh v0.8.2` |
...
```text
dist/earththrutime3d-image-v0.8.2.tar.gz
dist/earththrutime3d-data-v0.8.2.tar.gz
dist/earththrutime3d-host-v0.8.2.tar.gz
dist/SHA256SUMS-v0.8.2
```
...
scp dist/earththrutime3d-*-v0.8.2.tar.gz dist/SHA256SUMS-v0.8.2 dolfinid:~/earththrutime3d-release/
```
...
sha256sum -c SHA256SUMS-v0.8.2
docker load -i earththrutime3d-image-v0.8.2.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/earththrutime3d
tar -xzf earththrutime3d-host-v0.8.2.tar.gz -C /srv/earththrutime3d
mkdir -p /srv/earththrutime3d/data/v0.8.2 /srv/earththrutime3d/db \
         /srv/earththrutime3d/backups /srv/earththrutime3d/acme
tar -xzf earththrutime3d-data-v0.8.2.tar.gz -C /srv/earththrutime3d/data/v0.8.2
