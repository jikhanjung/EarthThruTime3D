...
def viewer_strings():
    return {
        ...
        "pinThen": _("오늘 {today} · 판 {plate} · {then}"),
        "pinHere": _("지금 {where}"),
        "pinCentre": _("이 핀이 가운데 오도록 화면을 돌립니다."),
        "pinRemove": _("{n}번 핀 지우기"),
        "pinGone": _("오늘 {today} · 판 {plate} · 이 땅은 모델에서 {from} Ma까지만 거슬러 갑니다."),
        "pinOff2002": _("오늘 {today} · 판 {plate} · 이 지도에서는 {reach} Ma 이전에 핀을 그리지 않습니다."),
        "pinOcean": _("여기에는 대륙 다각형이 없어 따라갈 판이 없습니다."),
        "pinDistance": _("{a}–{b} {km} km (오늘 {today} km)"),
